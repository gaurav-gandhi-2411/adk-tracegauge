...
**No shipped default changed as a result of this table.** `confidence=0.98` and `min_n=30` stay as shipped — this section replaces a misleading single number with an honest range, not a new tuning decision. See `docs/audit/AC1_SKEW_SENSITIVITY.md` and `docs/audit/AD2_REAL_CV_MEASUREMENT.md` for the investigation and a real (not assumed) CV measurement from an actual local-model run.
