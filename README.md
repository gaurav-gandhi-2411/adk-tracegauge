...
Every entry carries its own `source_url` and `fetched_on` date. 20 of the 22 entries are re-verified against the vendor's own live pricing page by `scripts/check_price_table_vs_vendor.py` (weekly, and before every release); the other 2 cannot be, and `report` says so whenever it prices with one: `gemini-2.0-flash` (retired 2026-06-01, no longer on Google's page — the rate is the last one published, kept for historical sessions) and `__local_zero_cost__` (a $0.00 policy for models you assert are local, not a vendor rate; its `source_url`/`fetched_on` are placeholders). **Prices change without notice** — verify against the source before relying on a number for a real budget decision. Two independent freshness guards: a per-entry `is_stale` check (past `STALE_THRESHOLD_DAYS=90`, warns loudly but still reports the number) at use time, and `.github/workflows/price-freshness.yml` running weekly in CI regardless of whether anyone's pushed a commit. See `_pricing.py`'s module docstring for the full detail (long-context tiering, cache-read discount verification, thinking-token billing, and the server-side built-in-tool tokens this package deliberately refuses to price rather than guess at).
...
Two commands, no files to create. This runs a deterministic, in-memory demo agent (bundled with the package — nothing is read from your machine) through a real `InMemoryRunner`, twice, with a deliberate cost regression injected into the second run, then fires the real `adk-tracegauge check` gate against it. **Measured, not estimated — and slower than an earlier version of this README claimed (78.2s, which could not be reproduced): median 438.5s (7.3 min) from an empty virtualenv to the printed regression verdict** (3 runs: 468.7s, 438.5s, 372.0s; Windows 11, Ryzen 7 6800H, Python 3.13.5, cold pip cache, home network, `google-adk==2.7.1`). **About 88% of that was pip installing `google-adk[eval]`'s 108-package dependency tree (329.4s of a 373.5s run); this package's own install is 5.5s and the demo itself ~30s.** Those numbers are for releases up to 0.8.x, which pulled that extra. From 0.9.0 the base install is bare `google-adk` (49 distributions, not 110): measured 2026-09-21 on the equivalent composition (bare `google-adk` 2.9.2 + adk-tracegauge 0.8.1 with `--no-deps`, Windows, Python 3.11, cold pip, single run each) at 68s versus 386s for the full tree, and 92 MB versus 737 MB installed; a re-measurement of the published 0.9.0 wheel is recorded in the CHANGELOG. Same exact output every run — see `examples/` for the full script this reuses. Raw logs and harness: [`reports/quickstart_wall_clock_2026-09-20/`](https://github.com/gaurav-gandhi-2411/adk-tracegauge/blob/main/reports/quickstart_wall_clock_2026-09-20/SUMMARY.md).
...
**What this gate reliably catches, stated first, not buried under the caveats below:** at real measured hosted-model variance (`gemini-3.5-flash-lite`, a genuine 36-case evalset — see the real-hosted-model section further down), the shipped paired default catches a **25% cost regression 98.00% of the time at `n=30`, the shipped minimum eval-set size** [97.29%, 98.53%] — a model swap, a new tool call added to every turn, a prompt-template change that meaningfully bloats context: the class of regression that actually happens in practice, not a constructed edge case. **No other cost-regression tool publishes a power number for its own gate at all** — stated once, plainly, not as a recurring comparison: this isn't a claim of being better than a specific named competitor, it's a claim that this number existing, measured and citable, is itself unusual for the category. The rest of this section is about the part that's genuinely harder — a fine-grained 10% drift needs a substantially larger eval set to catch reliably (below) — stated with the same rigor, not softened to protect the headline above.
...
## Sample completeness: is `n` what you actually asked for?

**This is not a bug-detection feature.** It is a validity precondition on this
package's own statistical output. `adk-tracegauge check`'s "achieved power" line
(above) is a real, honest statement about what the gate could reliably detect
— but only over the sample it actually ran. Until this feature, nothing in
this package could tell a genuinely complete `n` apart from one silently
shortened somewhere upstream: a dropped eval case looks, from `adk-tracegauge`'s
own vantage point, exactly like an eval set that legitimately has fewer
cases. `adk-tracegauge snapshot --eval-set-file <path>` (used alongside
`--eval-history`, required together — see below) closes that specific gap: it
checks the captured sample against the eval set's own definition, before you
ever get to `check`.

```bash
adk-tracegauge snapshot \
  --entrypoint my_eval_suite:run_and_return_store \
  --output current.json \
  --eval-history <path-to-adk-eval's-.evalset_result.json> \
  --eval-set-file <path-to-the-original-.evalset.json> \
...
have — pointing it at the wrong file (or a stale one from a different eval
set) yields confident nonsense: every real case looks "missing," even though
the run itself was fine. `evaluate_completeness` checks for this explicitly —
if **zero** expected case IDs match **any** observed record or skip, despite
...
stating plainly: `.evalset_result.json` (what `--eval-history` already reads,
for `eval_case_id` pairing) is *produced by the same pipeline this check
exists to verify*. A case silently dropped before producing any result is,
by construction, also absent from that file — treating it as ground truth
for "how many cases were expected" would be circular: it would agree with a
run that dropped a case just as readily as one that didn't, because it only
ever reflects what the pipeline *claims* it ran, never what it was *asked*
to run. The eval-set file is authored before any run happens and isn't
produced by anything this mechanism checks — it's the one independent source
of truth available.

**Scope, stated honestly against [`eval-defect-bench`](https://github.com/gaurav-gandhi-2411/eval-defect-bench)**,
the benchmark this workspace built from real, historical `google/adk-python`
bug-fix commits in this same subsystem: its `SILENT_ACCUMULATION_LOSS` shape
(`TAXONOMY.md`, n=6) covers exactly this class — "something that should have
contributed to the final verdict silently didn't, and nothing in the return
value signals that anything is missing." Completeness reconciliation covers
**2 of those 6** (`#3`, `#5` — the empty-collection-guard-non-execution
sub-mechanism in `AgentEvaluator.evaluate_eval_set`, the exact mechanism
reproduced above and the one `#6951` itself was filed against), not the
other 4, for two distinct reasons — not rounded up to "most of the shape":

- **`#18`, `#20`, `#27`** (the zip-truncation/`num_samples=0` sub-mechanism in
  `LlmAsJudge.evaluate_invocations`) are **structurally out of reach at this
  package's integration point.** Every registered `Evaluator` — including
  `adk-tracegauge`'s own `CostEfficiencyEvaluator` — receives the *same* full
  `actual_invocations` list from `LocalEvalService` for a given eval case.
  When a *sibling* metric silently drops an invocation from its own
  per-invocation results internally, `adk-tracegauge`'s own invocation count
  for that case is completely unaffected — it still receives, captures, and
  prices every real invocation normally. There is no API surface exposing
  another registered metric's own result count to this one; catching this
  sub-mechanism would need `LocalEvalService`-level introspection ADK doesn't
  expose, not a runtime signal reachable from a plugin/evaluator's own
  position in the pipeline.
- **`#23`** (unconditional overwrite in
  `EvaluationGenerator.convert_events_to_eval_invocations`, `user_content =
  event.content` silently discarding a valid earlier value) is out of reach
  for a *different* reason: it doesn't reduce invocation or case **count** at
  all. The invocation still exists, still reaches every registered
  evaluator, still gets counted correctly by this check — it just carries
  wrong *content*. A pure count/ID reconciliation, by construction, has
...
Registration uses `google.adk.evaluation.metric_evaluator_registry`, which google-adk marks `@experimental`. This package pins `google-adk>=2.6.0,<2.10.0` (and the same range for the `[eval]` extra) accordingly, re-validated on each bump — see `CHANGELOG.md`. If the registry API breaks in a future release, registration happens at import time as a side effect, so the failure mode is a loud, immediate error on `import adk_tracegauge`, not a silent no-op. The one thing that is tolerated is the registry module being *missing* (no `[eval]` extra, so no pandas): registration is then skipped, `adk_tracegauge.EVAL_REGISTRATION_SKIPPED_REASON` says why, and only `ModuleNotFoundError` is caught — an `AttributeError` or a moved symbol still fails the import. A CI job (`bare-adk`) runs the whole base surface on bare google-adk 2.6.0 and 2.9.2.
...
- **The above limitation is now surfaced at RUNTIME, every `adk-tracegauge check` run, not only in this doc (Phase 4 R4).** Every run prints an `achieved power` line — the minimum effect size the bootstrap test could reliably (80% power, the same bar B4 used above) detect given THIS run's own observed variance and `n` — and an explicit `WARNING` whenever your configured `--min-effect-usd`/`--min-effect-pct` floor is smaller than that achievable floor (see the Quickstart output above for a real example: at `n=40`, `BASE_SD≈$0.0015`/mean≈$0.0086, the achievable floor is ~$0.000536/6.25%, ABOVE the default `$0.0001` floor, so the WARNING fires). The achieved-power figure is a normal-approximation to the bootstrap CI (bootstrap power has no closed form) — validated against B4/R2's own measured grid at 7 points, accurate to within 2–8 percentage points, worst at `n=25` (see `_regression.py`'s "Achieved statistical power" section for the full accuracy table and derivation). **As of `0.5.0`, the same condition that fires the WARNING line above also sets a distinct process exit code, `4` (`EXIT_UNDERPOWERED_PASS`)** — `status="pass"` AND this run's own observed variance/`n` means the configured practical-significance floor could not reliably (80% power) be detected. **As of the release after `0.5.1`, this applies to BOTH modes, not two-sample only.** It was originally scoped to two-sample alone, on the reasoning that two-sample power at realistic variance could be as low as 4–9% (see the CV-sensitivity tables above). That scoping turned out to be the gap: real hosted-model measurement (`docs/audit/AD2_REAL_CV_MEASUREMENT.md`) found the shipped `--mode auto` DEFAULT — paired — returns a clean `status="pass"` at only **37.85% power** for a 10% regression at `n=30` (real measured within-case CV=0.1307 on a real `gemini-3.5-flash-lite` call), the exact low-resolution "pass" this exit code exists to flag, silently un-flagged because it happened in the mode most users actually run. The mechanism (`power_warning`) was already computed identically for both modes before this fix; only the exit-code escalation was mode-restricted. This is a real behavior change: **a CI config that treats any non-zero exit code as a build failure may see a previously-`0`-exiting, real-variance run now exit `4` in EITHER mode**, even with no regression found and no workload change — intentional (a "pass" at low power was misleading by omission), not a bug, but worth checking your own CI config against, especially if you rely on paired mode (the default). Full exit-code table in `_cli.py`'s module docstring.

**What `n` does paired mode actually need, at real measured hosted-model variance?** Swept `n` ∈ {50, 75, 100, 150, 200} at the real measured within-case CV (0.1307), same 2,000-trial/cell + Wilson-CI convention (`scripts/measure_ap2_n_sweep.py`, `reports/ap2_n_sweep.json`):

| n | 10% effect | 25% effect |
|---|---|---|
| 30 (shipped `min_n`) | 37.85% [35.75%, 40.00%] (from the point measurement above) | 98.00% [97.29%, 98.53%] |
| 50 | 58.50% [56.33%, 60.64%] | 99.90% [99.64%, 99.97%] |
| 75 | 74.75% [72.80%, 76.61%] | 100.00% [99.81%, 100%] |
| 100 | **87.20% [85.66%, 88.59%]** | 100.00% [99.81%, 100%] |
| 150 | 97.00% [96.16%, 97.66%] | 100.00% [99.81%, 100%] |
| 200 | 99.25% [98.77%, 99.54%] | 100.00% [99.81%, 100%] |

**Stated plainly: at real measured hosted-model variance, reliably (80% power) detecting a 10% cost regression needs roughly `n=100`** — the first swept value clearing 80% (74.75% at `n=75` still falls short). **A 25% regression is reliably caught at any `n` in this table, including the shipped `min_n=30` (98.00%).** This is the honest contrast AP2 asked for: the gate's real strength is catching regressions of the size that actually happen in practice (a model swap, a new tool call added to every turn — typically far more than 10%), not detecting a fine-grained 10% drift at a small eval set.

**`min_n=30` re-examined again against this real data — kept at 30, not raised.** Raising it to ~100 (what 10%-effect power would actually need) would refuse every real 30–99-invocation eval set outright — this project's own `examples/03_ci_regression_gate.py` deliberately uses `n=40`, just above the current floor, precisely because real ADK eval cases can involve real/expensive model calls, so teams keep eval sets to tens of cases, not hundreds (see "What this gate can and cannot detect" below). The same reasoning the original two-sample `min_n` re-examination already reached applies with more force here, now backed by real hosted-model data instead of only a synthetic grid: no single `min_n` generically solves "80% power for the regression size YOU care about" for every user's own variance and threshold. **The runtime signal — now firing in both modes as of this fix — carries that burden instead**: a real `n=30` paired run at real variance still computes a valid result and reports its own achieved power honestly (37.85% for a 10% floor, in the measurement above), with exit code 4 making that visible to CI without reading logs. Refusing to run below a much higher floor would trade a real, honestly-labeled low-power result for no result at all on eval sets that exist for a real reason (call cost) — worse, not better. **`min_n=30` was explicitly re-examined (4.3), not left unchanged by default** — real measurement at n∈{30,35,40,45} (10% effect, B4's generator, 200 trials/cell) showed 71.50% [64.88%, 77.30%] (143/200), 79.00% [72.84%, 84.07%] (158/200), 77.50% [71.23%, 82.74%] (155/200), 83.00% [77.18%, 87.57%] (166/200) detection, i.e. `n=30` genuinely doesn't clear 80% for this scenario either — but **kept at 30 anyway**: no single `min_n` generically solves "80% power for the regression size YOU care about" (that depends on your own cost variance and threshold, which this package cannot know in advance — B4's own grid shows even `n=100` only clears 64.50% [57.65%, 70.80%] (129/200 trials) for a 5% effect), so raising it would just trade real signal on legitimate 30–44-invocation eval sets for a false sense of a "fixed" problem. The runtime achieved-power/warning mechanism above is the actually-general fix. **(Superseded by Phase 5 S4, next bullet) False-positive rate at `n=30` under the ORIGINAL 0.95-confidence default measured at 500 trials: 4.60% [3.08%, 6.81%] (23/500); independent re-check, different seed, 500 trials: 4.20% [2.76%, 6.34%] (21/500)** — both well above the ~2.5% nominal expectation, because at this variance level the 5%-relative practical floor is only ~1.3 sampling standard errors from zero at `n=30` and doesn't meaningfully suppress noise-driven false positives on its own. **A BCa (bias-corrected/accelerated) bootstrap was implemented as an experiment and empirically measured (4.5)**: no measurable improvement (percentile vs. BCa FPR: 6.00% [3.83%, 9.28%] (18/300) vs. 5.33% [3.31%, 8.49%] (16/300) at `n=10`, 3.00% [1.59%, 5.60%] (9/300) vs. 3.33% [1.82%, 6.03%] (10/300) at `n=25`) — expected, since BCa's corrections target bias/skew in the bootstrap distribution, near-zero for a near-symmetric mean statistic on this project's cost data; NOT shipped. A studentized bootstrap was assessed but not built or tested — it needs a per-resample SE estimate that is known to be unstable at `n<20-30`, exactly the regime it would need to help in; see `_regression.py`'s "Anti-conservatism at small n" section for the full reasoning. This remains a real, honest, unresolved limitation at small `n`.
...
- **Grounding fees, server-side tool-use tokens, audio input and non-text output are flagged, not priced — the total says so.** Each is billed by the vendor on a basis the token arithmetic does not have, so the invocation's tokens are priced, the invocation is marked `*` in `report`, listed under "NOT included in the total", and `report --json` gets `total_is_complete: false` (the total is a lower bound; the `adk eval` metric reports `NOT_EVALUATED` for it instead of a score). The reasons: (0) **server-side tool-use tokens** (Google Search grounding, code execution; every real Gemini 2.5 grounded call carries them) are treated differently by vendors (Vertex states Google Search grounding input tokens are not charged), so they are left out, not guessed; (1) **Google Search grounding** is a per-prompt (Gemini 2.5: $35 per 1,000 grounded prompts) or per-query (Gemini 3.x: $14 per 1,000 search requests) fee, and a free allowance (1,500 prompts a day / 5,000 requests a month) that a plugin cannot see, so any figure would be a guess about your account; (2) **audio input** has its own published rate on several models (gemini-2.5-flash $1.00/M vs $0.30/M text; gemini-2.5-flash-lite $0.30 vs $0.10; gemini-3.1-flash-lite $0.50 vs $0.25), so pricing it at the text rate under-charges — the flag also applies to models whose page lists a single price, where the text rate may in fact be right; (3) **non-text output** (image, audio, video tokens in `candidates_tokens_details`) is published at a different, much higher output rate than text (e.g. Gemini image models $30-$60/M vs $1.50-$3 text), and a model id that prefix-matches a text entry (`gemini-2.5-flash-image` → `gemini-2.5-flash`) would otherwise be priced at the text output rate. Image, video and document **input** is priced at the text rate and not flagged. This was checked against the RAW vendor pages on 2026-09-21 (not a summary) and is recorded per model in `input_modality_verification` in the price table: where a Gemini page labels the modalities (gemini-2.5-flash, 2.5-flash-lite, 3.1-flash-lite, 3.5-flash-lite, and the retired 2.0-flash in its last archived capture) image and video share the text rate and only audio differs; for the other Gemini entries the page lists one input price with no per-modality rate; for Claude and GPT the vendor docs bill image tokens at the model's input price, and no table model lists a video-input rate. Source: ai.google.dev/gemini-api/docs/pricing, platform.claude.com/docs/en/build-with-claude/vision.md, developers.openai.com/api/docs/guides/images-vision.md, read 2026-09-21. The weekly vendor check now reports MISMATCH if a Gemini input row ever prices image or video apart from text. Prices for the flagged components are published, so pricing them is future work, not an impossibility. Calls made through LiteLlm (Claude, GPT, local) carry no modality breakdown in ADK's usage object, so audio or image tokens there cannot be told apart and are priced as ordinary input tokens.
- **Cache storage and cache writes are not priced.** A cached *read* is priced (a single 0.1× multiplier, verified against every vendor's published cached rate on 2026-09-20). Two other cache costs are not: **explicit-cache storage** (Gemini bills it per token per *hour* the cache exists — $0.50–$4.50 per 1M tokens per hour on the current page — and neither `usage_metadata` nor any per-call field carries a cache lifetime, so it cannot be derived from a run) and **cache-write surcharges** (Anthropic, and the OpenAI models that list a cache-write price, bill writes above the base input rate; ADK surfaces no cache-write token count). A workload that creates explicit caches will therefore cost more than this tool reports; treat its figure as the per-call token cost, not the whole bill.
- **Other pricing tiers are not modelled**: OpenAI's and Anthropic's long-context tiers, and every vendor's Batch/Flex/Priority tiers. Each price entry is the standard, short-context tier (plus Gemini's `> 200k` tier, which the table does model).
