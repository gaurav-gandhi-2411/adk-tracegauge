...
**The two modes need a different CV measured against your data, because pairing cancels a different noise source than two-sample sees.** Two-sample mode's power depends on the raw CV of your per-invocation costs (what you'd compute directly from a `snapshot`'s `cost_usd` values — includes both case-to-case cost differences and run-to-run noise). Paired mode cancels out case-to-case cost differences entirely (that's the whole mechanism pairing relies on — see "Shipped default" above), so its power depends instead on the **within-case** CV — how much cost varies for the *same* eval case across independent runs, a narrower, harder-to-observe-directly quantity. This is why the paired column below is not simply "better" at every CV value than two-sample at the same nominal CV: they are not measuring variability the same way, and a low-CV two-sample workload is not automatically a low-within-case-CV paired workload.

**Power to detect a true 10% cost regression, confidence=0.98, >=2,000 trials/cell, Wilson 95% CIs** (`scripts/measure_power_by_cv_grid.py`, full grid with CIs in `reports/power_by_cv_grid.json`):

**Two-sample (CV = raw per-invocation cost CV):**
...
**Find your column, don't guess it.** `adk-tracegauge check` already computes and prints this for YOUR actual data on every single run — the "achieved power" / "minimum reliably-detectable effect" line, derived from that run's own observed variance and `n` (`_regression.py`'s `minimum_detectable_effect_usd`, a normal approximation to the bootstrap CI), never from a constant or an assumption. Read that line before trusting either table above for your own workload — the tables show the shape of the tradeoff; your own run's printed line is the actual number that applies to you.

**A real, measured within-case CV — the number that actually applies to paired mode, the shipped default.** `scripts/measure_within_case_cv_ollama.py` ran the same 36-case evalset TWICE (real repeats of the SAME cases, not two different evalsets) against `ollama_chat/qwen2.5:7b`, and measured **within-case CV = 0.1566** (pooled duplicate-measurement estimator; bias check t-stat=-0.189, not significant — the two runs are exchangeable). Locating this on the table above (measured directly at n=30 and n=36, not interpolated — `scripts/measure_q1_within_case_power.py`): **power to detect a true 10% cost regression is 28.45% [26.52%, 30.47%] at n=30 (the shipped `min_n`), and 32.25% [30.24%, 34.33%] at n=36 (this evalset's actual size)** — both far below the 80%-power bar, and roughly 1/3 of a previous, unqualified "99.22%" figure this README used to publish. 25%-effect power stays strong (92.25%/96.35%). Domain of validity: one evalset, one local model, two repeats per case — see `docs/audit/Q1_WITHIN_CASE_CV.md` for the full investigation, including why skewness could not be estimated from two repeats and what sampling temperature Ollama used. **This apparent gap against the published grid was independently reconciled, not just asserted**: comparing both harnesses on the same scale-invariant footing found the published grid's own noise assumption corresponds to an average CV of ~5.7% — this evalset's real within-case CV (15.66%) is nearly 3x noisier, which fully accounts for the difference; neither harness was wrong. See `docs/audit/Q1A_RECONCILIATION.md` for the full reconciliation, including a scale-mismatch false start caught and discarded before publication.

**No shipped default changed as a result of this table.** `confidence=0.98` and `min_n=30` stay as shipped — this section replaces a misleading single number with an honest range, not a new tuning decision. See `docs/audit/AC1_SKEW_SENSITIVITY.md`, `docs/audit/AD2_REAL_CV_MEASUREMENT.md`, and `docs/audit/Q1_WITHIN_CASE_CV.md` for the investigation and real (not assumed) CV measurements from actual local-model runs.
