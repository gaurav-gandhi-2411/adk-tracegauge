...
- **Cache storage and cache writes are not priced.** A cached *read* is priced (a single 0.1× multiplier, checked against each vendor's published cached rate). Two other cache costs are not: **explicit-cache storage** (Gemini bills it per token per *hour* the cache exists — $0.50–$4.50 per 1M tokens per hour on the current page — and neither `usage_metadata` nor any per-call field carries a cache lifetime, so it cannot be derived from a run) and **cache-write surcharges** (Anthropic, and the OpenAI models that list a cache-write price, bill writes above the base input rate; ADK surfaces no cache-write token count). A workload that creates explicit caches will therefore cost more than this tool reports; treat its figure as the per-call token cost, not the whole bill.
- **Other pricing tiers are not modelled**: OpenAI's and Anthropic's long-context tiers, and every vendor's Batch/Flex/Priority tiers. Each price entry is the standard, short-context tier, and `scripts/check_price_table_vs_vendor.py` verifies exactly that (plus Gemini's `> 200k` tier, which the table does model).
