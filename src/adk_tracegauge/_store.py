...
class UsageStore:
    ...
    def record_session(self, invocation_id: str, session_id: str) -> None:
        ...
        captured by ``TraceGaugeUsagePlugin.before_run_callback`` (App+Plugin
        harnesses) AND ``after_model_callback`` (Phase 4 R2 -- the hook that
        actually fires during `adk eval`/``AgentEvaluator.evaluate()``; see
        ...
        ``eval_case_ids_by_session`` parameter) -- unlike ``invocation_id``,
        which google-adk always regenerates fresh and random on every run,
        confirmed by reading ``evaluation_generator.py``'s ``Event.new_id()``
        and ``runners.py``'s ``new_invocation_context_id()``. Note session_id
        itself is ALSO regenerated by `adk eval`'s default flow unless the
        eval case's own ``session_input.session_id`` is authored in the
        .evalset.json file (see google-adk's ``local_eval_service.py``) --
        this is exactly why eval case id, not session_id, is the PRIMARY
        pairing key as of Phase 4 R2; session_id remains a real, useful
        fallback for hand-rolled harnesses that pin it directly.
        """
        with self._lock:
